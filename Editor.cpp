...
int Editor::loadInputMesh (const string &fileName){
  ...
  {
	  vector<vtkIdType> temp;
	  tetNeighbors_.push_back(temp);
	  for (int j = 0; j < inputMesh_->GetCell(i)->GetNumberOfPoints(); j++)
	  {
		  vtkIdList* sortie;
		  vtkIdList* points;
		  points->InsertNextId(inputMesh_->GetCell(i)->GetPointId(j));
		  inputMesh_->GetCellNeighbors(i, points, sortie);
		  for (int k = 0; k < sortie->GetNumberOfIds(); k++)
		  {
			  if ((std::find(tetNeighbors_[i].begin(), tetNeighbors_[i].end(), sortie->GetId(i))) == tetNeighbors_[i].end()) {
				  tetNeighbors_[i].push_back(sortie->GetId(i));
			  }
		  }
	  }
  }
