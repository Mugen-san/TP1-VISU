...
int IsoSurfacer::ComputePartialIntersection(const int &tetId){
	...
	for (int i = 0; i < Input->GetCell(tetId)->GetNumberOfEdges(); i++)
	{
		if (IsCellOnLevelSet(Input->GetCell(tetId)->GetEdge(i)))
		{
			pair<vtkIdType, vtkIdType> newP;
			newP.first = Input->GetCell(tetId)->GetEdge(i)->GetPointId(0);
			newP.second = Input->GetCell(tetId)->GetEdge(i)->GetPointId(1);
			tetEdges.push_back(newP);
	...
	for (int i = 0; i < tetEdges.size() ; i++)
	{
		...
		int j = 0;
		while (j < neighbors.size() && !deja_traite)
		{
...
int IsoSurfacer::ComputeSimpleIntersection(vtkCell *tet){
	...
	for (int i = 0; i < tet->GetNumberOfEdges() ; i++)
	{
		...
		{
			pair<vtkIdType, vtkIdType> newP;
			newP.first = tet->GetEdge(i)->GetPointId(0);
			newP.second = tet->GetEdge(i)->GetPointId(1);
			tetEdges.push_back(newP);
			ReOrderTetEdges(tetEdges);
		}
	}
	...
	for (int i = 0; i < tetEdges.size() ; i++)
	{
		vector<double> p = ComputeEdgeIntersection(tetEdges[i]);
		createdPts->InsertNextId(Output->GetPoints()->InsertNextPoint(p[0], p[1], p[2]));
	}
