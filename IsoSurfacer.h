...
class VTK_EXPORT IsoSurfacer : public vtkAlgorithm {
  ...
  protected:
    ...
    //QUESTION 8
	  ...
	  double p1_v = Input->GetPointData()->GetScalars()->GetComponent(edge.first,0);
	  double p2_v = Input->GetPointData()->GetScalars()->GetComponent(edge.second,0);
	  
	  //Get coordinates of edge ends
	  double p1[3], p2[3];
	  Input->GetPoint(edge.first, p1);
	  Input->GetPoint(edge.second, p2);

	  //Compute the coordinate of the intersection 
	  double percent = (Value - p1_v) / (p2_v - p1_v);
	  p[0] = (p2[0] - p1[0])*percent + p1[0];
	  p[1] = (p2[1] - p1[1])*percent + p1[1];
	  p[2] = (p2[2] - p1[2])*percent + p1[2];
      return p;
    };
